...
class _GroupDetailScreenState extends State<GroupDetailScreen>
  ...
  Future<void> _shareSimplifiedDebts(List<dynamic> transactions) async {
    try {
      final boundary =
      ...
      final String groupName = _currentGroup.name.replaceAll(
        RegExp(r'[^a-zA-Z0-9]'),
        '',
      );
      final String timestamp = DateTime.now().millisecondsSinceEpoch.toString();
      final file = await File(
        '${tempDir.path}/${groupName}_$timestamp.png',
      ).create();
      await file.writeAsBytes(pngBytes);

      final text = _getSimplifiedDebtsText(transactions);

      await Share.shareXFiles(
        [XFile(file.path)],
        text: text,
        subject: 'Simplified Debts - ${_currentGroup.name}',
      );
    } catch (e) {
      debugPrint('Error sharing debts: $e');
    }
  }

  String _getSimplifiedDebtsText(List<dynamic> transactions) {
    if (transactions.isEmpty) {
      return 'Great news! All expenses in the group "${_currentGroup.name}" have been simplified. Now you can settle up easily! 🎉';
    }

    final buffer = StringBuffer();
    buffer.writeln('📊 *Simplified Debt Summary for ${_currentGroup.name}*');
    buffer.writeln('------------------------------------------');
    buffer.writeln(
      'We\'ve optimized the group expenses to minimize the number of transactions.',
    );
    buffer.writeln('Here are the easiest ways for everyone to settle up:');
    buffer.writeln();
    ...
    for (final tx in transactions) {
      ...
      final to = _members
          ...
          .name;
      buffer.writeln(
        '✅ *$from* pays *$to*:  ${_currencySymbol}${formatAmount(tx.amount)}',
      );
    }

    buffer.writeln();
    buffer.writeln('Generated by DUTCH - Your friendly expense manager.');
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  'Optimized group expenses into ${transactions.length} transactions.',
                ...
                if (transactions.isEmpty)
                  ...
                  Flexible(
                    child: Stack(
                      children: [
                        // 1. Share-only content (Captured by RepaintBoundary)
                        // Moved off-screen so it's not visible to the user but still painted for capture.
                        Positioned(
                          top: -5000,
                          left: 0,
                          right: 0,
                        ...
                        // 2. Real App UI (Visible to the user)
                        ListView.builder(
                          shrinkWrap: true,
                                ...
                                children: [
                                  ...
                                  SizedBox(
                                    ...
                                    child: Center(
                                      child: HugeIcon(
                                        icon:
                                            HugeIconsStrokeRounded.arrowRight02,
                                        size: 18,
                                        color: themeColor,
                                      ),
                                    ),
                    ...
                    if (transactions.isNotEmpty) ...[
                      Expanded(
                        child: OutlinedButton.icon(
                          onPressed: () => _shareSimplifiedDebts(transactions),
                          icon: HugeIcon(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 15),
          // Here we use a Column of items for the image Version (No Scrollbar)
          ...transactions.map((tx) {
            final from = _members
                .firstWhere(
                  (m) => m.email.toLowerCase() == tx.fromEmail.toLowerCase(),
